 ...
 */
var makeMessage = function makeMessage(update) {
    var line1 = ' Update available: ' + chalk.green.bold(update.latest) + chalk.dim(' (current: ' + update.current + ')') + ' ';
    var line2 = ' Run ' + chalk.green('npm install -g ' + update.name) + ' to update. ';
 ...
 */
var notifyUpdate = function notifyUpdate(cb) {
            ...
            if (update.latest !== update.current) {
                ...
                this.prompt(prompts).then(function (props) {
                    if (props.updateGenerator === 'yes') {
                        spawn('npm', ['install', '-g', 'generator-feathers'], { stdio: 'inherit' }).on('close', function () {
                            process.exit();
                        });
...
module.exports = {
    extend: function extend(generator) {
